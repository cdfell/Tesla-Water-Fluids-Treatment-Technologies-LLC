...
import { PromocodesListActionTypes, PromocodesListAction } from "types/redux/promocodes/promocodesList";
import { Dispatch } from "redux";

async function list(filter?: any) {
  const response = await axios.get(
    `/promocodes?page=${filter.page}&limit=${filter.limit}&promocodes=${filter.promocodes ? filter.promocodes : ''}`,
  );
  return response.data;
}

async function filterPromocodes(request: any, filter: any) {
  const response = await axios.get(`/promocodes?page=${filter.page}&limit=${filter.limit}${request}`);
  return response.data;
}
