...
import { CategoriesListActionTypes, CategoriesListAction } from "types/redux/categories/categoriesList";
import { Dispatch } from "redux";

async function list(filter?: any) {
  const response = await axios.get(
    `/categories?page=${filter.page}&limit=${filter.limit}&categories=${filter.categories ? filter.categories : ''}`,
  );
  return response.data;
}

async function filterCategories(request: any, filter: any) {
  const response = await axios.get(`/categories?page=${filter.page}&limit=${filter.limit}${request}`);
  return response.data;
}
